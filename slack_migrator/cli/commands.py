...
class MigrationOrchestrator:
    ...
    def check_unmapped_users(self, migrator_instance: SlackToChatMigrator) -> bool:
        ...
        return (hasattr(migrator_instance, 'unmapped_user_tracker') and 
                migrator_instance.unmapped_user_tracker.has_unmapped_users())
    
    def report_validation_issues(self, migrator_instance: SlackToChatMigrator, is_explicit_dry_run: bool = False) -> bool:
        """Report validation issues and ask user if they want to proceed anyway."""
        logger.info("")
        logger.info("🚨 VALIDATION ISSUES DETECTED!")
        logger.info(f"Found {migrator_instance.unmapped_user_tracker.get_unmapped_count()} unmapped user(s).")
        logger.info("")
        logger.info("⚠️  WARNING: If you proceed without fixing these mappings:")
        logger.info("   • Messages from unmapped users will be sent by the workspace admin")
        ...
        if is_explicit_dry_run:
            ...
            try:
                response = input("⚠️  Proceed anyway despite unmapped users? (NOT RECOMMENDED) (y/N): ").strip().lower()
                if response in ['y', 'yes']:
                    logger.warning("Proceeding with unmapped users - messages will be attributed to workspace admin")
                    return True
                else:
                    logger.info("Migration cancelled. Please fix the user mappings and try again.")
                    return False
    ...
    def report_validation_success(self, is_explicit_dry_run: bool = False):
        ...
        if is_explicit_dry_run:
            logger.info("You can now run the migration without --dry_run to perform the actual migration.")
    ...
    def run_validation(self) -> bool:
        ...
        try:
            ...
            logger.error("Please fix the issues identified during validation before proceeding.")
            sys.exit(1)
    ...
    def get_user_confirmation(self) -> bool:
        ...
        try:
            response = input("Proceed with the actual migration? (y/N): ").strip().lower()
        ...
        if self.args.dry_run:
            ...
            try:
                ...
                if self.check_unmapped_users(self.migrator):
                    if self.report_validation_issues(self.migrator, is_explicit_dry_run=True):
                        # User should not be able to proceed in explicit dry run mode
                        logger.info("Use normal migration mode to proceed with unmapped users.")
                        sys.exit(1)
                    else:
                        sys.exit(1)
                else:
...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--creds_path', required=True, 
    ...
    parser.add_argument('--workspace_admin', required=True, 
    ...
    parser.add_argument('--update_mode', action='store_true', 
    ...
    parser.add_argument('--skip_permission_check', action='store_true', 
