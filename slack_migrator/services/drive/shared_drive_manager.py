...
class SharedDriveManager:
    ...
    def validate_shared_drive(self, shared_drive_id: str) -> bool:
        ...
        try:
            ...
            log_with_context(
                logging.WARNING,
                f"Shared drive {shared_drive_id} not accessible: {e}"
            )
            return False
    ...
    def get_or_create_shared_drive(self) -> Optional[str]:
        ...
        try:
            ...
            if shared_drive_id:
                try:
                    drive_info = self.drive_service.drives().get(driveId=shared_drive_id).execute()
                    log_with_context(
                        logging.INFO,
            ...
            log_with_context(
                logging.ERROR,
                f"Failed to get or create shared drive: {e}"
            )
            return None
    ...
    def _find_or_create_shared_drive(self, drive_name: str) -> Optional[str]:
        ...
        try:
            ...
            log_with_context(
                logging.INFO,
                f"Searching for existing shared drive: {drive_name}"
            )
            ...
            log_with_context(
                logging.INFO,
                f"Creating new shared drive: {drive_name}"
            )
            ...
            created_drive = self.drive_service.drives().create(
                body=drive_metadata,
