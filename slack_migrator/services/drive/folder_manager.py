...
class FolderManager:
    ...
    @retry()
    def create_root_folder_in_shared_drive(self, folder_name: str, shared_drive_id: str) -> Optional[str]:
        ...
        try:
            ...
            log_with_context(
                logging.ERROR,
                f"Failed to create root folder in shared drive: {e}"
            )
            return None
    ...
    def create_regular_drive_folder(self, folder_name: str) -> Optional[str]:
        ...
        try:
            ...
            log_with_context(
                logging.INFO,
                f"Creating regular Drive folder: {folder_name}"
            )
            ...
            log_with_context(
                logging.ERROR,
                f"Failed to create regular Drive folder: {e}"
            )
            return None
            
    def get_or_create_channel_folder(self, channel: str, parent_folder_id: str, shared_drive_id: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            log_with_context(
                logging.INFO,
                f"Creating channel folder: {channel}",
                channel=channel
            )
    ...
    def get_channel_folder_id(self, channel: str, parent_folder_id: str, shared_drive_id: Optional[str] = None) -> Optional[str]:
    ...
    @retry()
    def set_channel_folder_permissions(self, folder_id: str, channel: str, user_emails: list, shared_drive_id: Optional[str] = None) -> bool:
        ...
        for email in user_emails:
            try:
                ...
                log_api_request("POST", "drive.permissions.create", permission, 
                              folder_id=folder_id, channel=channel)
