...
from slack_migrator.services.drive import SharedDriveManager, FolderManager, DriveFileUploader
...
class FileHandler:
    ...
    def __init__(self, drive_service, chat_service, folder_id: str, migrator, dry_run: bool = False):
        ...
        self.shared_drive_manager = SharedDriveManager(drive_service, migrator.config, dry_run=dry_run)
        self.folder_manager = FolderManager(drive_service, workspace_domain, dry_run=dry_run)
    ...
    def _initialize_shared_drive_and_folder(self) -> None:
        ...
        try:
            ...
            if not shared_drive_id and shared_drive_name:
                ...
                self._shared_drive_id = self.shared_drive_manager.get_or_create_shared_drive()
            ...
            if self._shared_drive_id:
                ...
                self._root_folder_id = self.folder_manager.create_regular_drive_folder("Imported Slack Attachments")
            ...
            # Final fallback
            self._root_folder_id = self.folder_manager.create_regular_drive_folder("Slack Attachments Fallback")
    ...
    def _pre_cache_root_folder(self) -> None:
        ...
        try:
            ...
            log_with_context(
                logging.DEBUG,
                f"Pre-cached {file_count} files from root folder"
            )
    ...
    def upload_attachment(self, file_obj: Dict, channel: Optional[str] = None, space: Optional[str] = None, user_service=None, sender_email: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if username and self.migrator and hasattr(self.migrator, 'is_external_user'):
            ...
            use_direct_upload = (
                mime_type in self.DIRECT_UPLOAD_MIME_TYPES and 
                actual_size <= self.DIRECT_UPLOAD_MAX_SIZE and
                not self.dry_run and
                self.chat_uploader.is_suitable_for_direct_upload(name, actual_size)
            )
            ...
            if use_direct_upload:
                ...
                direct_result = self._upload_direct_to_chat(file_obj, file_content, channel, space, user_service, sender_email)
                if direct_result:
            ...
            drive_result = self._upload_to_drive(file_obj, file_content, channel, sender_email)
            if drive_result:
                # Store result for future reference and update stats
    ...
    def _upload_direct_to_chat(self, file_obj: Dict, file_content: bytes, channel: Optional[str] = None, space: Optional[str] = None, user_service=None, sender_email: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if user_service:
                    ...
                    user_chat_uploader = ChatFileUploader(user_service, dry_run=self.dry_run)
                    upload_response, attachment_metadata = user_chat_uploader.upload_file_to_chat(
                        temp_file_path, name, space
                    )
                else:
                    # Use the default chat uploader (admin service)
                    upload_response, attachment_metadata = self.chat_uploader.upload_file_to_chat(
                        temp_file_path, name, space
                    )
                ...
                if upload_response and attachment_metadata:
                    ...
                    if user_service:
                        ...
                        attachment_ref = self.chat_uploader.create_attachment_for_message(
                            upload_response, attachment_metadata
                        )
    ...
    def _upload_to_drive(self, file_obj: Dict, file_content: bytes, channel: Optional[str] = None, sender_email: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            is_google_docs_link = ('docs.google.com' in url_private or 
            ...
            if is_google_docs_link:
                ...
                if 'docs.google.com/document' in url_private:
                ...
                elif 'docs.google.com/spreadsheets' in url_private or 'sheets.google.com' in url_private:
                    ...
                    if not mime_type or mime_type == 'application/octet-stream':
                        guessed_type, _ = mimetypes.guess_type(name)
                        mime_type = guessed_type if guessed_type else 'application/vnd.google-apps.document'
            ...
            if channel:
                ...
                if hasattr(self.migrator, "active_users_by_channel") and channel in self.migrator.active_users_by_channel:
                    ...
                    for user_id in active_users:
                        ...
                        if email:
                            ...
                            internal_email = self.migrator._get_internal_email(user_id, email)
                            if internal_email and internal_email not in user_emails_for_sharing:
                ...
                if folder_id and user_emails_for_sharing:
                    self.folder_manager.set_channel_folder_permissions(
                        folder_id, channel, user_emails_for_sharing, self._shared_drive_id
                    )
                    
                # Pre-cache file hashes from this folder to avoid duplicate uploads
                self.drive_uploader.pre_cache_folder_file_hashes(folder_id, self._shared_drive_id)
            ...
            try:
                ...
                drive_file_id, public_url = self.drive_uploader.upload_file_to_drive(
                    temp_file_path, name, folder_id, self._shared_drive_id,
                ...
                if (user_email and not self.migrator._is_external_user(user_email) and 
                    not self._shared_drive_id):
                ...
                drive_result = {
                    'type': 'drive',
                    'link': public_url or f"https://drive.google.com/file/d/{drive_file_id}/view",
                    'drive_id': drive_file_id,
                    'name': name,
                    'mime_type': mime_type
                }
    ...
    def get_file_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'successful_uploads': self.file_stats['drive_uploads'] + self.file_stats['direct_uploads'],
    ...
    def _download_file(self, file_obj: Dict) -> Optional[bytes]:
        ...
        try:
            ...
            is_google_docs = (
                ('docs.google.com/document' in url_private) or
                ('docs.google.com/spreadsheets' in url_private) or
                ('docs.google.com/presentation' in url_private) or
                ('sheets.google.com' in url_private and '/edit' in url_private) or
                ('slides.google.com' in url_private and '/edit' in url_private)
            )
            
            # Check if this is a Google Drive file that we should reference directly
            is_google_drive_file = (
                'drive.google.com/file/d/' in url_private or
                'drive.google.com/open?id=' in url_private
            )
    ...
    def _create_drive_reference(self, file_obj: Dict, channel: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    def share_file_with_members(self, drive_file_id: str, channel: str) -> bool:
        ...
        try:
            ...
            for user_id in active_users:
                email = self.migrator.user_map.get(user_id)
                if email:
                    emails_to_share.append(email)  # Include ALL users, both internal and external
