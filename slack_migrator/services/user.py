...
def generate_user_map(export_root: Path, config: Dict) -> Tuple[Dict[str, str], List[Dict[str, Any]]]:
    ...
    for user in users:
        ...
        if ignore_bots and user.get('is_bot', False):
            ...
            logger.info(f"Ignoring bot user {user_id} ({user.get('real_name', user.get('name', 'Unknown'))}) - ignore_bots enabled")
            continue
    ...
    if users_without_email:
        ...
        for user in users_without_email:
            ...
            logger.warning(f"  - {user_type}: {user['name']} (ID: {user['id']}){deleted_status}")
        
        logger.warning("\nTo map these users, add entries to user_mapping_overrides in config.yaml:")
        for user in users_without_email:
            deleted_comment = " # DELETED USER - still referenced in messages" if user.get('deleted', False) else f" # {user['name']}"
    ...
    for override_user_id, override_email in user_mapping_overrides.items():
        if override_user_id not in user_map:
            ...
            logger.info(f"Added user mapping override for {override_user_id} -> {override_email} (not in users.json)")
