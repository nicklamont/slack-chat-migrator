...
class ChatFileUploader:
    ...
    def upload_file_to_chat(self, file_path: str, filename: str, parent_space: Optional[str] = None) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
        ...
        if self.dry_run:
            return (
                ...
                {
                    "name": filename,
                    "driveFile": {"name": f"DRY_CHAT_FILE_{filename}"}
                }
            )
        ...
        try:
            ...
            log_api_request("POST", "chat.media.upload", {
                "filename": filename,
                "mimeType": mime_type,
                "size": file_size
            })
            ...
            media = MediaFileUpload(
                file_path, 
                mimetype=mime_type,
                resumable=True if file_size > 5 * 1024 * 1024 else False  # Use resumable for files > 5MB
            )
            ...
            upload_request = self.chat_service.media().upload(
                ...
                media_body=media,
                body={"filename": filename}  # Filename in request body as per Chat API requirements
            )
    ...
    def create_attachment_for_message(self, upload_response: Dict[str, Any], 
        ...
        if "driveDataRef" in upload_response or "driveFileId" in upload_response:
            ...
            if "driveDataRef" in upload_response and "driveFileId" in upload_response["driveDataRef"]:
                ...
                attachment = {
                    "driveDataRef": {
                        "driveFileId": upload_response["driveFileId"]
                    }
                }
                log_with_context(
                    logging.DEBUG,
