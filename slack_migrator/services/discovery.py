    ...
    try:
        ...
        while True:
            ...
            for space in response.get("spaces", []):
                ...
                space_name = space.get("name", "")  # This is the format "spaces/{space_id}"
            ...
            if len(spaces) == 1:
                ...
                for space_info in spaces:
                    try:
                        ...
                        members_response = migrator.chat.spaces().members().list(
                        ...
                        if "memberships" in members_response:
                            space_info["member_count"] = len(members_response.get("memberships", []))
                            
                            # If we need more members, we could paginate here
                            if "nextPageToken" in members_response:
                                space_info["member_count"] = str(space_info["member_count"]) + "+"
...
def get_last_message_timestamp(migrator, channel: str, space: str):
    ...
    try:
        ...
        request = migrator.chat.spaces().messages().list(
            parent=space,
            pageSize=1,
