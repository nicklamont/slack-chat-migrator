...
class UnmappedUserTracker:
    ...
    def __init__(self):
        ...
        self.user_contexts: Dict[str, Set[str]] = defaultdict(set)  # Track where they were encountered
    ...
    def track_unmapped_mention(self, user_id: str, channel: str = "", message_ts: str = "", text: str = ""):
...
def log_unmapped_user_summary_for_dry_run(migrator) -> None:
    ...
    """
    if not hasattr(migrator, 'unmapped_user_tracker') or not migrator.unmapped_user_tracker.has_unmapped_users():
    ...
    log_with_context(
        logging.ERROR,
        f"🚨 DRY RUN DETECTED {len(unmapped_users)} UNMAPPED USERS 🚨"
    )
    ...
    for user_id in unmapped_users:
        ...
        context_info = f" # Found in: {', '.join(contexts)}" if contexts else " # Unmapped user"
        ...
        log_with_context(
            logging.ERROR, 
            f'  "{user_id}": "user@yourdomain.com"{context_info}'
        )
    ...
    if user_analysis:
        ...
        if analysis_summary.get('Bots and workflow automations', 0) > 0:
            ...
            log_with_context(logging.ERROR, "   OPTION 1 (Easiest): Enable 'ignore_bots: true' in config.yaml")
            log_with_context(logging.ERROR, "   OPTION 2: Map to archive account 'bot-archive@yourdomain.com'")
            log_with_context(logging.ERROR, "   NOTE: Bot integrations won't work in Google Chat anyway")
            log_with_context(logging.ERROR, "")
            
        if analysis_summary.get('Deleted users', 0) > 0:
            log_with_context(logging.ERROR, "🗑️  For DELETED USERS:")
            log_with_context(logging.ERROR, "   RECOMMENDED: Map to 'deleted-user@yourdomain.com'")
            log_with_context(logging.ERROR, "")
            
        if analysis_summary.get('Users without email addresses', 0) > 0:
            log_with_context(logging.ERROR, "📧 For USERS WITHOUT EMAIL:")
            log_with_context(logging.ERROR, "   RECOMMENDED: Find their real email or use placeholder")
            log_with_context(logging.ERROR, "")
    ...
    has_bots = any(user_analysis.get(uid, {}).get('type') in ['bot', 'workflow_bot'] for uid in unmapped_users)
    if has_bots:
    ...
    for user_id in unmapped_users:
        ...
        if user_type in ['bot', 'workflow_bot']:
            log_with_context(logging.ERROR, f'  "{user_id}": "bot-archive@yourdomain.com"  # {user_name} (bot)')
        elif user_type == 'deleted_user':
            log_with_context(logging.ERROR, f'  "{user_id}": "deleted-user@yourdomain.com"  # {user_name} (deleted)')
        else:
            log_with_context(logging.ERROR, f'  "{user_id}": "user@yourdomain.com"  # {user_name} ({user_type})')
    
    log_with_context(logging.ERROR, "")
    log_with_context(logging.ERROR, "💡 WHY IGNORE BOTS:")
    log_with_context(logging.ERROR, "   • Most bot content is automated notifications or integrations")
    log_with_context(logging.ERROR, "   • Bot functionality won't work in Google Chat anyway")
    log_with_context(logging.ERROR, "   • Focus on human conversations that matter")
    log_with_context(logging.ERROR, "   • Simplifies migration by removing mapping requirements")


def analyze_unmapped_users(migrator, unmapped_user_ids: list) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        if not users_file.exists():
            log_with_context(logging.WARNING, "users.json not found, cannot analyze unmapped users")
            return analysis
...
def categorize_user_analysis(user_analysis: Dict[str, Dict[str, Any]]) -> Dict[str, int]:
...
def scan_channel_members_for_unmapped_users(migrator) -> None:
    ...
    try:
        ...
        if not channels_file.exists():
            log_with_context(logging.WARNING, "channels.json not found, skipping channel member validation")
            return
        ...
        if migrator.config.get('include_channels'):
            ...
            channels_to_check = [ch for ch in channels_data if ch.get('name') in include_set]
        else:
            # Check all channels except excluded ones
            exclude_set = set(migrator.config.get('exclude_channels', []))
            channels_to_check = [ch for ch in channels_data if ch.get('name') not in exclude_set]
        ...
        if ignore_bots:
            try:
                ...
                log_with_context(logging.WARNING, f"Error loading users.json for bot checking: {e}")
                user_lookup = {}
        ...
        log_with_context(
            logging.ERROR,
            f"Error scanning channel members for unmapped users: {e}"
        )
