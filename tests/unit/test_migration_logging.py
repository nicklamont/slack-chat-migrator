...
def _make_tracker(unmapped_count: int = 0) -> UnmappedUserTracker | None:
    """Build an UnmappedUserTracker, or None if count is 0."""
    if unmapped_count == 0:
        return None
    tracker = UnmappedUserTracker()
    for i in range(unmapped_count):
        tracker.add_unmapped_user(f"U{i:03d}")
    return tracker
...
class TestCollectStatistics:
    ...
    def test_unmapped_users_counted(self):
        ...
        stats = _collect_statistics(state, tracker)
    ...
    def test_handles_none_unmapped_user_tracker(self):
        """If the tracker is None, unmapped_users is 0."""
        state = _make_state()
        stats = _collect_statistics(state, None)
        assert stats["unmapped_users"] == 0
...
class TestLogMigrationSuccess:
    ...
    def test_normal_success(self, mock_log):
        ...
        log_migration_success(state, dry_run=False, duration=120.0)
    ...
    def test_no_work_interrupted_early(self, mock_log):
        ...
        state = _make_state()
        log_migration_success(state, dry_run=False, duration=5.0)
    ...
    def test_no_work_but_channels_processed(self, mock_log):
        ...
        state = _make_state(channels_processed=["general"])
        log_migration_success(state, dry_run=False, duration=10.0)
    ...
    def test_duration_logged(self, mock_log):
        ...
        log_migration_success(state, dry_run=False, duration=90.0)
    ...
    def test_zero_duration(self, mock_log):
        ...
        log_migration_success(state, dry_run=False, duration=0.0)
    ...
    def test_very_long_duration(self, mock_log):
        ...
        log_migration_success(state, dry_run=False, duration=7200.0)
    ...
    def test_no_issues_logged_when_clean(self, mock_log):
        ...
        log_migration_success(state, dry_run=False, duration=60.0)
    ...
    def test_dry_run_skips_space_message_stats(self, mock_log):
        ...
        log_migration_success(state, dry_run=True, duration=10.0)
...
class TestLogMigrationFailure:
    ...
    def test_runtime_error(self, mock_log):
        ...
        log_migration_failure(state, False, exc, duration=45.0)
    ...
    def test_dry_run_failure(self, mock_log):
        ...
        log_migration_failure(state, True, exc, duration=2.0)
    ...
    def test_progress_before_failure(self, mock_log):
        ...
        log_migration_failure(state, False, RuntimeError("oops"), duration=60.0)
    ...
    def test_progress_before_interruption(self, mock_log):
        ...
        log_migration_failure(state, False, KeyboardInterrupt(), duration=15.0)
    ...
    def test_different_exception_types(self, mock_log):
        ...
        for exc, expected_type in exceptions:
            mock_log.reset_mock()
            state = _make_state()
            log_migration_failure(state, False, exc, duration=1.0)
            first_call = mock_log.call_args_list[0]
    ...
    def test_traceback_logged_on_error(self, mock_log, mock_format_exc):
        ...
        state = _make_state()
        log_migration_failure(state, False, RuntimeError("fail"), duration=1.0)
    ...
    def test_traceback_not_logged_on_interrupt(self, mock_log, mock_format_exc):
        ...
        state = _make_state()
        log_migration_failure(state, False, KeyboardInterrupt(), duration=1.0)
